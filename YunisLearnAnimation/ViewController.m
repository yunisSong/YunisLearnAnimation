...
- (NSArray *)sourceArray{
    if (_sourceArray == nil) {
        _sourceArray = ({
            NSArray *arr = @[@"点击显示扩散动画",@"学习彦祖类似购物车动画",@"简单的画线与路径动画",@"qq音乐转场动画",@"3d转场动画",@"另外一种画线方式",@"系统动画旋转角度的思考",@"印象笔记动画效果OC版本",@"仿light弹出提示框",@"半糖动画效果",@"Light下拉首页图片变形",@"light太阳运行动画",@"仪表盘",@"UICollectionView",@"组合动画"];
            arr;
        });
    }
